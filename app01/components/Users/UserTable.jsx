...
import { View, Text, TextInput, TouchableOpacity, Switch, CheckBox, FlatList, StyleSheet } from 'react-native';
      ...
      renderItem={({ item }) => (
        <View style={styles.userRow}>
          ...
          <View style={styles.actions}>
            <TouchableOpacity style={styles.editButton} onPress={() => onEdit(item.uuid)}>
              <Text>Modificar</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.deleteButton} onPress={() => onDelete(item.uuid)}>
              <Text>Eliminar</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  userRow: { flexDirection: 'row', justifyContent: 'space-between', padding: 10, borderBottomWidth: 1, borderBottomColor: '#ccc' },
  userName: { fontSize: 16, fontWeight: 'bold' },
  rolesContainer: { flexDirection: 'row', gap: 5 },
  roleBadge: { backgroundColor: '#eee', padding: 5, borderRadius: 5 },
  actions: { flexDirection: 'row', gap: 10 },
  editButton: { padding: 5, backgroundColor: '#007BFF', borderRadius: 5 },
  deleteButton: { padding: 5, backgroundColor: '#FF0000', borderRadius: 5 }
});